...
@Component({
  selector: 'app-tour-filters-page',
  standalone: true,
  imports: [
    FormsModule,
    NgFor,
  ...
  template: `
    <h1 class="page-title">Tour filters</h1>
    <p class="page-intro">
      Define dropdown buckets per city. Then open each tour under <strong>Tours</strong> and tick which bucket(s) it appears in — independent of the tour’s technical “type” field.
    </p>
    <app-help-panel title="How this works">
      <app-admin-field-hint [text]="hint.tours" />
    </app-help-panel>
    ...
    <div class="field">
      <label class="field-label-with-hint">
        City
        <app-admin-field-hint text="Same destination as under Cities — URL segment after /city/ for the tours listing on the public site." />
      </label>
    ...
    <app-admin-modal [(open)]="modalOpen" [title]="editingId ? 'Edit tour filter' : 'New tour filter'">
      <div class="modal-scroll-form">
        ...
        <div class="field">
          <label>Slug</label>
          <input [(ngModel)]="form.slug" placeholder="e.g. water-sports" />
        </div>
        <div class="field">
...
export class TourFiltersPageComponent implements OnInit {
  ...
  reloadFilters(): void {
    ...
    this.http
      .get<AdminPaginated<CatalogFilterRow>>(
        ...
        { params: { domain: 'TOURS', page: this.page, pageSize: this.pageSize } },
      )
      .subscribe({
        next: (r) => {
  ...
  saveFilter(): void {
    ...
    if (!title || !slug) {
      this.error = 'Please enter a title and a slug.';
      return;
    }
    this.error = '';
    const body =
      this.editingId == null
        ? { title, slug, domain: 'TOURS' as const }
